        ...
        if dataelem:
            if dataelem.value.lower() == identifier.lower():
                for private_tag_element in range(
                    dataelem.tag.elem * 0x0100, dataelem.tag.elem * 0x0100 + 0x0100
                ):
...
def tag_image(file_path, group, identifier, tag_value):
    dicomfile, private_tag = add_private_tag(
        pydicom.dcmread(file_path), group, identifier, tag_value
    )
...
def tag_folder(dir_path, group, identifier, tag_value):
    ...
    for image_file in os.listdir(dir_path):
        try:
            tags.append(
                tag_image(
                    os.path.join(dir_path, image_file), group, identifier, tag_value
                )
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The group of the private element tag",
    )
    parser.add_argument(
        "--identifier", "-i", type=str, required=True, help="Identifier"
    )
    parser.add_argument(
        "--tag-value",
        "-t",
        type=str,
        required=True,
        help="String to tag the dicom files",
    )
